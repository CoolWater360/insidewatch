...
CREATE TABLE IF NOT EXISTS filings (
    ...
    -- ── Claim lease token ──────────────────────────────────────────────────
    -- Generated by claim_filing() when a worker wins the claim race and
    -- cleared (set to NULL) by complete_filing(), fail_filing(), skip_filing(),
    -- and reap_stale_filings().  Workers must echo the token they received
    -- back to every lifecycle-ending call; the UPDATE WHERE clause rejects
    -- the call if the token no longer matches, preventing a stale worker from
    -- overwriting a live worker's filing state.
    claim_token                 UUID,
...
BEGIN
    ...
    SET
        ...
        claim_token        = gen_random_uuid(),   -- new per-claim lease token
...
BEGIN
    ...
    SET
        status             = CASE
                                 ...
                                 ELSE 'failed'
                             END,
        claim_token        = NULL,    -- invalidate the stale worker's lease
        last_error         = format(
