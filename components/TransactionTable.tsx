...
export function TransactionTable({ rows, current, basePath }: Props) {
  if (rows.length === 0) {
    return (
      <div className="glass-card rounded-xl p-10 text-center">
        <p className="text-sm text-muted">No transactions match your filters.</p>
        <p className="mt-1 text-xs text-muted/60">Try broadening the date range or removing the direction filter.</p>
      </div>
    );
  }
        ...
        <thead>
          <tr className="border-b border-white/[0.07] bg-navy-900/60">
            ...
            <th className="px-4 py-3 text-left sm:hidden">
              <span className="sort-link">Transaction</span>
            </th>
            {/* Desktop headers */}
            <th className="hidden px-4 py-3 text-left sm:table-cell">
              <SortLink column="transaction_date" label="Date" current={current} basePath={basePath} />
            </th>
            <th className="hidden px-4 py-3 text-left sm:table-cell">
              <span className="sort-link">Company</span>
            </th>
            <th className="hidden px-4 py-3 text-left sm:table-cell">
              <span className="sort-link">Insider</span>
            </th>
            <th className="hidden px-4 py-3 text-left lg:table-cell">
              <span className="sort-link">Role</span>
            </th>
            {/* Type always visible */}
            <th className="px-4 py-3 text-left">
              <SortLink column="direction" label="Type" current={current} basePath={basePath} />
            </th>
            <th className="hidden px-4 py-3 text-right md:table-cell">
              <SortLink column="quantity" label="Qty" current={current} basePath={basePath} align="right" />
            </th>
            <th className="hidden px-4 py-3 text-right lg:table-cell">
              <SortLink column="unit_price" label="Price" current={current} basePath={basePath} align="right" />
            </th>
            {/* Value always visible */}
            <th className="px-4 py-3 text-right">
              <SortLink column="total_value" label="Value" current={current} basePath={basePath} align="right" />
            </th>
            {/* Src — desktop only */}
            <th className="hidden px-4 py-3 text-right sm:table-cell">
              <span className="sort-link">Src</span>
            </th>
          </tr>
        </thead>
